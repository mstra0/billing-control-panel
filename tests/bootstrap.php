...
function assert_contains($needle, $haystack, $message = '') {
    ...
    $_test_results['errors'][] = array(
        ...
        'actual' => is_string($haystack) ? substr($haystack, 0, 100) . (strlen($haystack) > 100 ? '...' : '') : var_export($haystack, true)
    );
    return false;
}
...
function assert_array_has_key($key, $array, $message = '') {
    ...
    $_test_results['errors'][] = array(
        ...
        'actual' => is_array($array) ? 'keys: ' . implode(', ', array_keys($array)) : var_export($array, true)
    );
    return false;
}
...
function run_test($name, $callback) {
    ...
    try {
        ...
        if ($_test_results['failed'] > $start_failed) {
            echo "FAIL\n";
        } else if ($_test_results['passed'] > $start_passed) {
            echo "PASS\n";
        } else {
            echo "NO ASSERTIONS\n";
        }
