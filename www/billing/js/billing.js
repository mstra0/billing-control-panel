...
function showAjaxError(containerId, message) {
  ...
  if (el)
    el.innerHTML =
      '<div class="ajax-error"><div class="flash flash-error">' +
      escapeHtml(message) +
      "</div></div>";
...
function buildPagination(pag, baseUrl, params) {
  ...
  if (pag.has_prev) {
    html += '<a href="' + escapeHtml(buildUrl(cur - 1)) + '">&laquo; Prev</a>';
  } else {
    html += '<span class="disabled">&laquo; Prev</span>';
  }
  var range = 2,
    start = Math.max(1, cur - range),
    end = Math.min(total, cur + range);
  if (start > 1) {
  ...
  for (var i = start; i <= end; i++) {
    if (i === cur) {
      ...
      html += '<a href="' + escapeHtml(buildUrl(i)) + '">' + i + "</a>";
    }
  }
  if (end < total) {
  ...
  if (pag.has_next) {
    html += '<a href="' + escapeHtml(buildUrl(cur + 1)) + '">Next &raquo;</a>';
  } else {
...
function pollJob(jobId) {
  var pollInterval = setInterval(function () {
    ...
    xhr.open(
      "GET",
      API_BASE + "?action=job_status&id=" + encodeURIComponent(jobId),
    );
    xhr.onload = function () {
      try {
      ...
      if (job.total_steps > 0) {
        stepText =
          "Step " + job.progress + " of " + job.total_steps + ": " + stepText;
      }
      ...
      if (job.status === "failed") {
        ...
        resultEl.style.display = "";
        resultEl.innerHTML =
          '<div class="flash flash-error">Error: ' +
...
var TOAST_RELOAD_DELAY_MS = 1200;
...
function showToast(message, type, options) {
  ...
  var autoDismiss =
    options.autoDismiss !== undefined ? options.autoDismiss : type !== "error";
...
function dismissToast(toast) {
  ...
  toast.className =
    toast.className.replace("toast-visible", "") + " toast-fade-out";
  setTimeout(function () {
    if (toast.parentNode) {
