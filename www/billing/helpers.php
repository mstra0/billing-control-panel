...
{
    $billing_data = dirname(dirname(__DIR__)) . "/billing";

    if (MOCK_MODE) {
        return $billing_data . "/test_shared";
    }
    ...
    if (!is_dir(SHARED_BASE_PATH) || !is_writable(SHARED_BASE_PATH)) {
        $fallback = $billing_data . "/test_shared";
        if (!is_dir($fallback)) {
...
function _fix_production_share($path, $subdirs)
{
    ...
    if (is_dir($path) && !is_readable($path)) {
        ...
        $mount_script =
            dirname(dirname(__DIR__)) . "/billing/scripts/mount_share.sh";
